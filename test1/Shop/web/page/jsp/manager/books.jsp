    ...
    <table>
        ...
        <div>
            <tr>
                ...
                <td><a href="/Shop/booksServlet?action=updateBook&id=<%=book.getId()%>">修改</a>
                    <a href="/Shop/booksServlet?action=deleteBook&id=<%=book.getId()%>">删除</a></td>
            </tr>
        </div>
        <% }
        %>
    </table>
    <a href="/Shop/page/jsp/manager/insert_book.jsp">添加图书</a>
    <%
        int end = 0;
        if (books.size() % 5 == 0) {
            end = books.size() / 5;
